...
const footerLinks = [
  ...
  {
    title: "Loại phim",
    items: typelistApi.items().map(({ name, slug }) => ({
      name,
      href: `/danh-sach/${slug}`,
    })),
  },
  ...
  return (
    <footer className="border-t border-border bg-background/50 backdrop-blur">
      <div className="container mx-auto px-4  py-12">
        <div className="grid gap-10 lg:grid-cols-[1.3fr_2fr]">
          ...
          <div className="space-y-5">
            ...
            <div className="flex flex-wrap gap-2">
              <div className="rounded-full border border-border bg-zinc-800 px-3 py-1 text-xs text-zinc-300">
                Full HD
              </div>
          ...
          <div className="grid gap-8 sm:grid-cols-2">
            {footerLinks.map((group) => (
              <div
                key={group.title}
                className="rounded-2xl border border-border/60 bg-zinc-800 p-5"
              >
                ...
                <div className="grid grid-cols-2 gap-3">
                  {group.items.map((item) => (
                    <Link
                      key={item.href}
                      href={item.href}
                      className="group flex items-center gap-2 text-sm text-zinc-300"
                    >
        ...
        {/* Bottom */}
        <div className="mt-10 gap-3 border-t border-border pt-5 text-xs text-zinc-300 md:flex-row md:items-center md:justify-between">
